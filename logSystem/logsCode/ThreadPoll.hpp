...
// 用于处理函数对象和可调用对象
#include<functional>
// 定义了一系列标准异常类，用于处理运行时错误
#include<stdexcept>

class ThreadPool{
private:
    std::vector<std::thread> workers;           //线程们
    // 表示一个可以调用且不接受任何参数、不返回任何值的函数对象
    std::queue<std::function<void()>> tasks;    //任务线程
...
public:
    ...
    // 调用 condition.wait(lock, [this]{ return this->stop || !this->tasks.empty(); }) 
    // 使线程进入等待状态，直到满足以下两个条件之一：线程池停止（stop 为 true）或任务队列不为空。
