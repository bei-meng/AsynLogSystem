...
namespace storage{
    ...
    static std::string UrlDecode(const std::string &str){
        // 比如将"Hello%20World%21"解码为"Hello World!"
        std::string res = "";
    ...
    public:
        ...
        int64_t FileSize(){
            ...
            if(ret == -1){
                mylog::GetLogger("asynclogger")->Info("%s, Get file size error: %s",filename_.c_str(),strerror(errno));
                return -1;
            }
            return s.st_size;
        }
        ...
        bool GetPosLen(std::string &content,size_t pos,size_t len){
            ...
            // 读入content
            ifs.seekg(pos,std::ios::beg);//文件指针移动到pos处
            content.resize(len);
        ...
        bool ScanDirectory(std::vector<std::string> &arry){
            for(auto &p : fs::directory_iterator(filename_)){
                ...
                arry.emplace_back(fs::path(p).relative_path().string());
            }
            return true;
        }
    };//FileUtil
    ...
    public:
        ...
        static bool UnSerialize(const std::string &str,Json::Value *val){
            // 建造者生成->建造者实例化json读对象->调用读对象的接口进行序列化写入val
