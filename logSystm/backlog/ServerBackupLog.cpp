...
std::mutex file_mutex; // 全局互斥锁

const std::string filename = "./logfile.log";
void usage(std::string procgrass){
    std::cout<<"usage error: "<<procgrass<<" port "<<std::endl;
}
...
void backup_log(const std::string &message){
    ...
    int write_bytes = fwrite(message.c_str(),1,message.size(),fp);
    if(write_bytes!=message.size()){
        perror("fwrite error: ");
        assert(false);
    }
    fflush(fp);
